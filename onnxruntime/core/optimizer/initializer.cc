...
struct Sqrt {
  void operator()(Tensor& tensor) const {
    ...
    for (auto& dst : span) {
      ...
      // v * v does produce test expected result
      dst = T(std::sqrt(v));
    }
  }
};
...
template <typename T>
struct ScaleByAxis {
  void operator()(Tensor& data, const Tensor& scaler, const int64_t block_size, const int64_t num_blocks) const {
    const auto scaler_size = scaler.Shape().Size();
    ToNumeric<T> to_numeric;
    T* dst = data.MutableData<T>();
    const T* src = scaler.Data<T>();
    int64_t block_offset = 0;
    for (size_t i = 0; i < static_cast<size_t>(num_blocks); i++, block_offset += block_size) {
      const size_t scaler_index = scaler_size == 1 ? 0 : i;
      const auto numeric_scaler = to_numeric(src[scaler_index]);
      for (int64_t j = 0; j < block_size; ++j) {
        const auto k = block_offset + j;
        dst[k] = T(to_numeric(dst[k]) * numeric_scaler);
      }
    }
  }
};
...
void Initializer::scale_by_axis(const Initializer& other, int axis) {
  ...
  utils::MLTypeCallDispatcher<MLFloat16, BFloat16, float, double, int32_t, int64_t> t_disp(data_.GetElementType());
  t_disp.Invoke<ScaleByAxis>(data_, other.data_, block_size, num_blocks);
}
